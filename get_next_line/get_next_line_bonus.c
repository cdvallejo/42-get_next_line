...
/*   Updated: 2022/05/17 11:01:00 by cvallejo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line_bonus.h"

//int	open(const char *file_route, int flag);
...
char	*ft_read_and_buff(int fd, char *text_asked)
{
	...
	while (!ft_strchr(text_asked, '\n') && rd_byte != 0)
	{
		...
		if (rd_byte < '\0')
		{
			return (ft_no_text_asked(buffer_text, text_asked));
		}
...
char	*ft_next_text_asked(char *text_asked)
{
	...
	next_text = malloc(ft_strlen(text_asked) - i + 1);
	if (!next_text)
		return (NULL);
	i++;
	j = 0;
...
{
	char		*line;
	static char	*text_asked[4096];

	if (fd < 0 || BUFFER_SIZE <= 0 || fd > 4096)
		return (NULL);
	text_asked[fd] = ft_read_and_buff(fd, text_asked[fd]);
	if (!text_asked[fd])
		return (NULL);
	line = ft_get_line(text_asked[fd]);
	text_asked[fd] = ft_next_text_asked(text_asked[fd]);
	return (line);
}
